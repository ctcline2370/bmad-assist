...
"""
    ...
    def test_validate_epic_scope_only_ignores_unrelated_prior_epic_teardown_drift(
        self, cli_runner: CliRunner, project_dir: Path
    ) -> None:
        """Test scoped validation does not require unrelated prior epic teardown."""
        (project_dir / "docs" / "epics" / "epic-2-foundation.md").write_text(
            """---
epic_num: 2
title: Foundation
status: done
...
### Story 2.1: Foundation Setup
Description.
"""
        )
        (project_dir / "docs" / "epics" / "epic-11-operations.md").write_text(
            """---
epic_num: 11
title: Operations
status: in-progress
...
### Story 11.5: Concurrency Aware Foundation Operations
Description.
"""
        )
        (
            ...
            """# Generated by bmad-assist
generated: 2026-04-22T00:00:00
project: epic-11-worktree

development_status:
  epic-2: done
  2-1-foundation-setup: done
  epic-2-retrospective: done
  epic-11: in-progress
  11-5-concurrency-aware-foundation-operations: in-progress
"""
        )
        (
            project_dir
            ...
            / "implementation-artifacts"
            / "code-reviews"
            / "synthesis-2-1-20260422T000000Z.md"
        ).write_text("# Review\n")
        (project_dir / ".bmad-assist" / "state.yaml").write_text(
            """version: 1
current_epic: 11
...
"""
        ...
        global_result = cli_runner.invoke(
            app,
            ["sprint", "validate", "--project", str(project_dir), "--format", "json"],
        )
        assert global_result.exit_code == EXIT_ERROR, global_result.output

        scoped_result = cli_runner.invoke(
            app,
            [
                "sprint",
                "validate",
        ...
        assert scoped_result.exit_code == EXIT_SUCCESS, scoped_result.output

        output = scoped_result.stdout
        json_start = output.find("{")
        if json_start == -1:
            pytest.fail(f"No JSON object found in output: {output}")
        ...
        assert not any(d["key"] == "loop-state" for d in data["discrepancies"])
...
class TestSprintHelp:
    ...
    def test_validate_help(self, cli_runner: CliRunner, cli_isolated_env: Path) -> None:
        ...
        assert "--epic-scope-only" in result.output
        assert "--verbose" in result.output
        assert "--format" in result.output
