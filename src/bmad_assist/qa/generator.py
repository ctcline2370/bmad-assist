...
def _get_epic_trace(
    ...
    The trace handler saves files as trace-{epic_id}-{timestamp}.md in the
    implementation-artifacts/traceability/ directory (via get_paths().output_folder).
    We check both that location (glob for timestamped files) and the legacy
    qa-artifacts path for backwards compatibility.
    ...
    """
    # Primary: check implementation-artifacts/traceability/ (where TraceHandler saves)
    try:
        from bmad_assist.core.paths import get_paths
        paths = get_paths()
        trace_dir = paths.output_folder / "traceability"
        if trace_dir.is_dir():
            # Glob for timestamped trace files: trace-{epic_id}-*.md
            matches = sorted(
                trace_dir.glob(f"trace-{epic_id}-*.md"),
                key=lambda p: p.stat().st_mtime,
                reverse=True,
            )
            if matches:
                logger.debug("Epic trace found: %s", matches[0])
                return matches[0]
    except RuntimeError:
        logger.debug("Paths not initialized, skipping primary trace lookup")

    # Fallback: check legacy qa-artifacts path
    trace_path = get_trace_path(config, project_path, epic_id)
    if trace_path.exists():
        logger.debug("Epic trace found (legacy path): %s", trace_path)
        return trace_path

    logger.info("No epic trace for %s", epic_id)
    return None
...
def _run_qa_plan_workflow(
    ...
    try:
        ...
        # Resolve provider and timeout using phase_models (respects bmad-assist.yaml overrides)
        phase_config = get_phase_provider_config(config, "qa_plan_generate")
        if isinstance(phase_config, list):
            # Multi-provider config — use first entry for single-LLM invocation
            provider_name = phase_config[0].provider
            model_name = phase_config[0].model
        else:
        ...
        # Invoke LLM
        logger.info("Invoking LLM for QA plan generation (model=%s, timeout=%ds)...", model_name, timeout)
