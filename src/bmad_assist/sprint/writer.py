...
# Keep sprint-status entries single-line for deterministic diffs and editor
# hygiene checks. Ruamel otherwise wraps long keys as "key: " plus an indented
# scalar on the next line, which leaves trailing whitespace after the colon.
...
def _strip_trailing_line_whitespace(path: Path) -> None:
    """Remove trailing spaces and tabs from a generated YAML file."""
...
def _write_with_ruamel(
    ...
    try:
        ...
        _strip_trailing_line_whitespace(temp_path)
